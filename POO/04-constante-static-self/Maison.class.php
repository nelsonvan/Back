...
class Maison
{
    ...
    // méthode qui appaertient à l'objet
...
echo "Nombre de porte de la maison : <strong>" . $maison->getNbPorte() . '</strong><hr>';
    ...
    // echo $maison->espaceTerrain . '<hr>'; /!\ erreur !! il n'est pas possible d'acceder a une propriété static via l'objet

    echo $maison->getNbPiece() . '<hr>'; // /!\ fonctionne mais pas logique !! bonne ecriture : Maison::getNbPiece()

    // echo Maison::$couleur; /!\ erreur !! on ne doit pas appeler une propriété non static par la classe
