...
--11-- Changer le prénom du stagiaire qui a l'id 8 de *Patriiiick* à *Patrick*

UPDATE stagiaires SET prenom = 'patriiiiiick' WHERE id_stagiaire = '8';



--12-- Rajouter dans la table manger que Patrick a mangé 5 Tagada purpule le 15 septembre

INSERT INTO manger (id_bonbon,id_stagiaire, date_manger, quantite) VALUES ('2','8', '2019-09-15', '15');
...
SELECT prenom, id_stagiaire FROM stagiaires WHERE id_stagiaire = '5';
...
SELECT prenom, yeux FROM stagiaires ORDER BY genre DESC, yeux;
...
SELECT  prenom, yeux FROM stagiaires WHERE yeux = 'marron' ORDER BY LIMIT 4;
...
SELECT  prenom, yeux, id_stagiaire FROM
...
SELECT nom, prenom, quantite FROM manger INNER JOIN stagiaires ON stagiaires.id_stagiaire = 
manger.id_stagiaire INNER JOIN bonbons ON manger.id_bonbon = bonbons.id_bonbon;
-- OU --
SELECT nom, prenom, quantite FROM manger AS m, bonbons AS b, stagiaires AS s WHERE s.id_stagiaire =
m.id_stagiaire AND b.id_bonbon = m.id_bonbon;
...
SELECT prenom, nom, date_manger FROM stagiaires AS s, manger AS m, bonbons AS b  
WHERE s.id_stagiaire = m.id_stagiaire AND  b.id_bonbon = m.id_bonbon;

--42-- Afficher les quantités consommées par les stagiaires (uniquement ceux qui ont mangé !)


SELECT prenom, quantite FROM stagiaires AS s, manger AS m, bonbons AS b  
WHERE s.id_stagiaire = m.id_stagiaire AND  b.id_bonbon = m.id_bonbon;
-- OU --
SELECT prenom, nom, quantite FROM stagiaires INNER JOIN bonbons INNER JOIN manger
WHERE manger.id_bonbon = bonbons.id_bonbon AND manger.id_stagiaire =
stagiaires.id_stagiaire AND quantite > 0;
...
SELECT m.quantite, COUNT(*) AS nb_conso, s.prenom FROM manger m INNER JOIN 
stagiaires s ON m.id_stagiaire = s.id_stagiaire GROUP BY s.prenom;


--44-- Afficher combien de bonbons ont été consommés au total
SELECT COUNT('tagada') FROM bonbons;
SELECT SUM(quantite) FROM stagiaires INNER JOIN bonbons INNER JOIN manger 
WHERE manger.id_bonbon = bonbons.id_bonbon AND manger .id_stagiaire =
 stagiaires.id_stagiaire AND nom = 'tagada';
...
SELECT prenom FROM stagiaires s, bonbons b, manger m WHERE m.id_bonbon =
b.id_bonbon AND m.id_stagiaire = s.id_stagiaire AND quantite < 1;
